...
#	If it didn't cover enough percents, redo with larger tolerance
	...
	edge_tol = .1
	#To identify high, thin peaks
	peak_tol = 3*mag_tol*corr
#	The bar between noise and events
	event_tol = mag_tol * corr
#	Lower threshold for when the event ends
	event_tol2 = event_tol * .5
#	Threshold for accumulated area under the curve to classify as a peak
...
#							If long enough, use increased tolerance to step backwards
							if(cuttime > 2):	
								...
								while(backwards>start and not done):
									backwards -= 1
									cuttime = HJD[tempend]-HJD[backwards]
									cutbase2 = eventline + (evmin-eventline)*0.01*(100-j*percentcut)+base_tol*min(cuttime/2,3)
									endgoal = min(1+int(np.log(tempend-backwards)),8)
	...
	if(events[pspl]+events[binary]+events[burst]+events[pbinary]+events[square]+events[longpspl]>1):
		...
		longpspls.append(str(filenumber))
	else:
	...
	data_line += '\r\n'
	outfile.write(data_line)
data = ['PSPL',str(len(pspls)),'Binary',str(len(binaries)),'Burst',str(len(bursts)),'Pbinary',str(len(pbinaries)),'Square',str(len(squares)),'Periodic',str(len(periodics)),'Long PSPL',str(len(longpspls)),'Flat',str(len(flats)),'No Base',str(len(nobases))]
data_line = ''
for value in data:
